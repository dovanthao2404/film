...
class TypeFilm extends Component {
  ...
  renderFilm = () => {
    ...
    const { typeFilm } = this.props.filmDetailCurrent;
    console.log(typeFilm)
    if (database) {
      const totalFilm = database.phim[typeFilm]?.length || this.totalFilm();
      const row = 3;
      var startPage = (pwdPagination - 1) * row * 4;
      var endPage = startPage + 12;
      endPage = endPage > totalFilm ? totalFilm : endPage;
      let listFilm = []
      if (typeFilm !== "tatcaphim") {
        for (let i = startPage; i < endPage; i++) {

          const film = database.phim[typeFilm][i];
          listFilm.push(<FilmItem key={i} imageUrl={film.imageUrl} title={film.title} type={typeFilm} />);
        }
      } else {
