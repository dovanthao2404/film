...
class Details extends Component {
  ...
  renderBackgroundCarousel = () => {
    ...
    for (let type in dataFilm.phim) {
      ...
      if (flag) {
        this.handleAddListFilmConnection();
        break
      };
    }
  }

  handleAddListFilmConnection = () => {
    const { dataFilm } = this.state;
    const { filmDetailCurrent } = this.props;
    console.log(dataFilm.phim[filmDetailCurrent.type]])
    const { name } = this.props.match.params;
    const LENGTH_CONNECTION = 8;
    for (let i = 0; i < LENGTH_CONNECTION; i++) {
      const filmCurrent = dataFilm.phim[filmDetailCurrent.type][i];
      if (this.toSlug(filmCurrent.title) !== name) {
        this.props.addFilmConnection(filmCurrent)
      }
    }
  }
