...
class WatchFilm extends Component {
  ...
  componentDidMount = async () => {
    ...
    const { url } = this.props.filmDetailCurrent;
    if (!url) {
      const url = document.URL || window.location.href;
      const elementUrl = url.split("/")

      const film = { ...this.state.dataFilm.phim[elementUrl[3]][elementUrl[5]] };

      if (film.episode.length > 0) {
        film.url = film.episode[elementUrl[6]].url
      }
      film.typeFilm = elementUrl[3];
      film.index = elementUrl[5];
      film.episode = elementUrl[6];

      this.handleClickEpisode(film)
    }
  ...
  renderEpisode = () => {
    ...
    if (this.props.filmDetailCurrent.index !== undefined && this.state.haveData) {
      ...
      console.log(this.state.dataFilm.phim[typeFilm])
      console.log(index)
      console.log(this.state.dataFilm.phim[typeFilm][index])
