...
class WatchFilm extends Component {
  ...
  renderEpisode = () => {
    let episode = [];
    const { typeFilm, index } = this.props.filmDetailCurrent;
    if (this.props.filmDetailCurrent && this.state.haveData) {
      console.log('davao')
      console.log(this.state.dataFilm.phim[typeFilm][index].episode)
      if (this.state.dataFilm.phim[typeFilm][index].episode.length > 1) {
