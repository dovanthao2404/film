...
export default class ListType extends Component {
  ...
  renderFilm = (_type, quantity) => {
    let listFilm = [];
    const { listFilmData } = this.state.dataFilm.phim;
    let i = 0;
    for (let type in listFilmData) {
      if (i > quantity)
        if (type === _type) {
          listFilm.push(<FilmItem imageUrl={listFilmData.type[i].imageUrl} title={listFilmData.type[i].title} />)
        } else {
