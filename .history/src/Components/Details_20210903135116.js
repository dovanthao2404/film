...
class Details extends Component {
    ...
    for (let type in dataFilm.phim) {
      for (let i = 0; i < dataFilm.phim[type].length; i++) {
        if (name === this.toSlug(dataFilm.phim[type][i].title)) {
          ...
          this.props.changeTitle(dataFilm.phim[type][i].title)
  ...
  renderConnection = () => {
    ...
    let flag = false;
    for (let i = 0; i < dataFilm.phim[filmDetailCurrent.type].length; i++) {
      if (name === this.toSlug(dataFilm.phim[filmDetailCurrent.type][i].title)) {
        this.props.changeUrl(dataFilm.phim[filmDetailCurrent.type][i].imageUrl)
        this.props.changeTitle(dataFilm.phim[filmDetailCurrent.type][i].title)
        this.props.changeType(filmDetailCurrent.type)
        flag = true;
        break;
      }
    }
          ...
          <div className="container">
            <div className="row">
              <div className="col col-md-3">
                <img className="img-fluid rounded" src={filmDetailCurrent.imageUrl} alt="..." />
