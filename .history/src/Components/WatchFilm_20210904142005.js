...
class WatchFilm extends Component {
  ...
  handleClickEpisode = (filmNext) => {
    const { typeFilm, index, title, url, episode } = filmNext;
    this.props.changeImageUrl(imageUrl)
    this.props.changeTitle(title)
    this.props.changeType(filmType)
    if (.episode?.length > 0) {
      this.props.changeUrl(episode[0].url)
    } else {
  ...
  renderEpisode = () => {
    let episodes = [];
    const { typeFilm, index, title, url, episode } = this.props.filmDetailCurrent;
    if (this.props.filmDetailCurrent.index !== undefined && this.state.haveData) {
      ...
      if (totalEpisode > 1) {
        ...
        for (let i = 0; i < totalEpisode; i++) {
          const filmNext = { ...this.props.filmDetailCurrent, episode: i }
