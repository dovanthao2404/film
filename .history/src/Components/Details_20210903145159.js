...
class Details extends Component {
  ...
  componentDidMount = () => {
    try {
      axios({
        ...
        this.setState({ dataFilm2: resp.data });
      })
  ...
  renderBackgroundCarousel = () => {
    const { dataFilm2 } = this.state;
    const { name, filmType } = this.props.match.params;
    for (let i = 0; i < dataFilm2.phim[filmType].length; i++) {
      const filmCurrent = dataFilm2.phim[filmType][i]
  ...
  //   const { dataFilm2 } = this.state;
  //   console.log(dataFilm2)
  //   const { filmType } = this.props.match.params;
  //   const listFilmConnection = [];
  //   const LENGTH_CONNECTION = 8;
  //   for (let j = 0; j < LENGTH_CONNECTION; j++) {
  //     const filmCurrent = dataFilm2.phim[filmType][j];
  //     listFilmConnection.push(<FilmItem imageUrl={filmCurrent.img}
  //     />)
  //   }
