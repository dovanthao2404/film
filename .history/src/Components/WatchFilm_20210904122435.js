...
class WatchFilm extends Component {
  ...
  renderEpisode = () => {
    let episode = [];
    const { typeFilm, index, title } = this.props.filmDetailCurrent;
    if (this.props.filmDetailCurrent && this.state.haveData) {
      const listEpisode = this.state.dataFilm.phim[typeFilm][index].episode;
      const totalEpisode = totalEpisode.length;
      if (totalEpisode > 1) {
        for (let i = 0; i < totalEpisode; i++) {
          <Link to={"/" + typeFilm + "/" + this.toSlug(title) + "/" + index} onClick={this.handleClickEpisode} className="btn btn-primary">Tập {i + 1}</Link>
        }
      }
